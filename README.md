...
_The following guide was put together for the London Plaform User Group, Nov, 2024._
...
```bash
git clone https://github.com/lukehinds/lopug.git && cd lopug
```
...
```bash
minder profile apply -f lopug.yaml
```
...
```bash
minder profile status list --detailed  --name lopug-profile
```
🙌 All good!
...
```bash
sed -i '' 's/remediate: "off"/remediate: "on"/' lopug.yaml
```
